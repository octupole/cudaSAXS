...
def model(q, params, constrained=True):
    ...
    for k in range(3):
        for kp in range(3):
            result += (rho[k] * rho[kp] * sigma[k] * sigma[kp] * 
...
def fit_saxs(q, I_exp, release_constraints=False):
    ...
    result = minimize(objective, initial_guess, args=(q, I_exp, True), 
    ...
    if release_constraints:
        ...
        bounds_unconstrained = [(0, None), (0, None), (None, 0), 
                                (None, None), (0, None), (None, None),
                                (0, None), (0, None), (0, None)]
        
        result = minimize(objective, initial_guess_unconstrained, 
                          args=(q, I_exp, False), method='L-BFGS-B', 
...
def main():
    parser = argparse.ArgumentParser(description='Fit SAXS data using a three-shell model.')
    parser.add_argument('-f', '--file', required=True, help='Input file with q and I data')
    parser.add_argument('-p', '--plot', action='store_true', help='Plot the original data and fit')
    parser.add_argument('-r', '--release', action='store_true', help='Release constraints after initial fit')
    ...
    if args.output:
        ...
        np.savetxt(args.output, np.column_stack((q_out, I_out)), 
                   delimiter='\t', header='q\tI(q)_fit', comments='')
