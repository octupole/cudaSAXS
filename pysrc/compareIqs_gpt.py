...
def fit_alpha(I_exp, I_sol, q):
    ...
    def objective(alpha):
        ...
        penalty = np.sum((diff[diff <= 10.0] - 10.0)**2) * 1e6  # Strong penalty for violating the constraint
...
def plot_functions_and_difference(q, I_exp, I_sol, difference, alpha):
    ...
    plt.plot(q, alpha * I_sol, label=f'alpha * I_sol(q) (alpha={alpha:.4f})', color='green')
...
def main():
    parser = argparse.ArgumentParser(description='Process and fit I(q) functions.')
    parser.add_argument('F_exp', type=str, help='Path to the experimental data file.')
    parser.add_argument('F_sol', type=str, help='Path to the solution data file.')
    parser.add_argument('--output', type=str, default='difference.txt', help='Output file name for the difference data.')
    parser.add_argument('--q_min', type=float, default=0.0, help='Minimum q value for interpolation grid.')
    parser.add_argument('--q_max', type=float, default=2.5, help='Maximum q value for interpolation grid.')

    parser.add_argument('--q_step', type=float, default=0.01, help='Step size for the interpolation grid.')
    
    parser.add_argument('--fit_q_min', type=float, default=1.7, help='Minimum q value for fitting.')
    parser.add_argument('--fit_q_max', type=float, default=2.5, help='Maximum q value for fitting.')
    parser.add_argument('--q_max_diff', type=float, default=2.0, help='Maximum q value for the difference function output.')
    parser.add_argument('--plot', action='store_true', help='Plot the difference and the functions in log scale.')
    ...
    alpha = fit_alpha(I_exp_interp[fit_mask], I_sol_interp[fit_mask], q_grid[fit_mask])
    ...
    np.savetxt(args.output, output_data, fmt='%.6f', header='q Difference', comments='')
    ...
    if args.plot:
        plot_functions_and_difference(q_grid[diff_mask], I_exp_interp[diff_mask], I_sol_interp[diff_mask], difference, alpha)
